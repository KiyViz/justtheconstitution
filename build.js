...
function main() {
  ...
  // ── Turnstile sitekey check ──
  // The placeholder ships a form nobody can submit: no widget renders, so no
  // token reaches the Worker and every submission is rejected. Warn loudly.
  const infoOut = path.join(ROOT, 'en', 'info', 'index.html');
  if (fs.existsSync(infoOut) && fs.readFileSync(infoOut, 'utf8').includes('TURNSTILE_SITEKEY_HERE')) {
    console.log(
      '\n  warning: Turnstile sitekey placeholder still in templates/info.template.html.' +
      '\n           The contact form will reject every submission until it is replaced' +
      '\n           with the real sitekey from the Cloudflare Turnstile dashboard.'
    );
  }

  // ── Minification (requires esbuild) ──
  // .min.* files are deploy-time artifacts: gitignored, regenerated by
  // Cloudflare Workers Builds during `npm run build`, then uploaded as
  // static assets. Never commit the .min.* outputs.
  if (esbuild) {
    console.log('\n── Minifying assets ──');
    let totalSaved = 0;
